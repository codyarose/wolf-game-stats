...
function useMyWool() {
  ...
  console.log("data", woolData);

  useEffect(() => {
    const getWool = async () => {
      if (currentAccount && contract) {
        const gweiResult = await contract.balanceOf(currentAccount) as BigNumber;
        const amount = gweiResult.toNumber() * Math.pow(10, -18);
